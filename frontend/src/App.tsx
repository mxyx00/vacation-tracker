...
function App() {
  ...
  useEffect(() => {
    ...
    fetch(`http://localhost:5175/api/timeoffrequests/employee/${selectedEmployeeId}`)
      .then(response => response.json())
  ...
  useEffect(() => {
    fetch("http://localhost:5175/api/timeoffrequests")
      ...
      .catch(error => console.error("Error loading all requests:", error));
  }, []);
  ...
  // Submit a new request or edit an existing request
  ...
  // Fill form with an existing request
  ...
  const approvedRequests = allRequests.filter(
    request => request.status === 1 && parseDate(request.startDate) <= timelineEnd
  );

  let timelineStart: Date | null = null;

  if (approvedRequests.length > 0) {
    const startDates = approvedRequests.map(request =>
      parseDate(request.startDate).getTime()
    );

    timelineStart = new Date(Math.min(...startDates));
  }
  ...
  function getTimelineStyle(request: TimeOffRequest) {
    ...
    const offsetMilliseconds = requestStart.getTime() - timelineStart.getTime();
    const daysFromTimelineStart = Math.round(offsetMilliseconds / millisecondsPerDay);

    const requestMilliseconds = requestEnd.getTime() - requestStart.getTime();
    const numberOfRequestDays = Math.round(requestMilliseconds / millisecondsPerDay) + 1;
      ...
      {approvedRequests.length === 0 ? (
        ...
        <div className="timeline">
          ...
          {approvedRequests.map(request => (
            <div className="timeline-row" key={request.id}>
              ...
              <div className="timeline-track">
                <div
                  ...
                  style={getTimelineStyle(request)}
                ></div>
              </div>
            </div>
          ))}
